	...
	private void Update () {
		...
		*/
		...
		else 
		{
			...
			//Toggle bool
			outOfAmmo = false;
			//anim.SetBool ("Out Of Ammo", false);			INTERESTING
			anim.SetLayerWeight (1, 0.0f);
		}

		//Shooting 
		if ((Input.GetMouseButtonDown (0) || Input.GetKeyDown ("joystick button 2"))&& !outOfAmmo && !isReloading && !isInspecting && !isRunning) 
		{
			anim.Play ("Fire", 0, 0f);
		...
		//Reload 
		if ((Input.GetKeyDown (KeyCode.R) || Input.GetKeyDown ("joystick button 4") ) && !isReloading && !isInspecting) 
		{
			//Reload
			Reload ();
